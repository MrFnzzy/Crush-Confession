...
export default function BackgroundMusicManager() {
  ...
  useEffect(() => {
    fetch("/api/music", { cache: "no-store" })
      ...
      .then((data) => {
        setSavedName(data?.music?.fileName || "");
        if (typeof data?.music?.volume === "number") setVolume(data.music.volume);
      })
      .catch(() => undefined);
  }, []);
      ...
      {savedName && (
        <div className="glass mt-6 rounded-2xl p-6 sm:p-8">
          ...
          <p className="mt-3 text-xs leading-relaxed text-muted">
            Past 100% the track is genuinely boosted louder than its normal maximum — useful for a quiet
            recording, but very high boosts can sound distorted. {volumeSaving && "Saving..."}
          </p>
