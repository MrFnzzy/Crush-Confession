...
// Tenor (Google) is an alternative provider — set NEXT_PUBLIC_TENOR_API_KEY
...
// Klipy is a third provider — free-for-life, built as a near drop-in
// replacement for Giphy/Tenor (it's who Discord/WhatsApp/Bluesky have been
// migrating GIF search to). No public demo key exists, so it's only used
// once you set your own free key from https://partner.klipy.com.
const KLIPY_KEY = process.env.NEXT_PUBLIC_KLIPY_API_KEY?.trim() || "";
// An arbitrary stable-ish per-app id Klipy asks for; not a secret.
const KLIPY_CUSTOMER_ID = "unspoken-confession-wall";

// Priority when more than one key is set: Klipy > Tenor > Giphy (Klipy's
// free tier has no rate-limit headaches; Giphy's fallback key is shared
// and gets throttled, so it's the last resort).
const PROVIDER: "klipy" | "tenor" | "giphy" = KLIPY_KEY ? "klipy" : TENOR_KEY ? "tenor" : "giphy";
...
/**
 * Klipy's public docs describe the endpoint/param shape (below) but don't
 * fully spell out the exact keys inside each item's `files` object the
 * way Giphy/Tenor's docs do. Rather than hard-code a guess that might
 * silently return no image, this walks the object looking for any string
 * value that looks like a real gif/webp/mp4 URL and prefers an actual
 * `.gif`. If Klipy's shape doesn't match once you test with a real key,
 * console.log(data) here once to see the real field names and adjust.
 */
function findMediaUrl(node: unknown, preferGif: boolean): string | undefined {
  if (typeof node === "string") {
    if (!/^https?:\/\//.test(node)) return undefined;
    if (preferGif && !/\.gif(\?|$)/i.test(node)) return undefined;
    if (!preferGif && !/\.(gif|webp|mp4)(\?|$)/i.test(node)) return undefined;
    return node;
  }
  if (Array.isArray(node)) {
    for (const item of node) {
      const found = findMediaUrl(item, preferGif);
  ...
  if (node && typeof node === "object") {
    for (const value of Object.values(node)) {
      const found = findMediaUrl(value, preferGif);
      if (found) return found;
    }
  }
  return undefined;
...
async function fetchFromKlipy(term: string, signal: AbortSignal): Promise<GifResult[]> {
  ...
  const endpoint = `${base}/${term.trim() ? "search" : "trending"}?${params.toString()}`;
  const res = await fetch(endpoint, { signal });
  if (!res.ok) throw new Error("klipy fetch failed");
  const data = await res.json();
  const items = Array.isArray(data?.data?.data) ? data.data.data : [];
  return items
    .map((gif: any) => ({
      id: String(gif.id ?? gif.slug ?? findMediaUrl(gif.files, true) ?? Math.random()),
      title: gif.title || gif.alt_text || "gif",
      thumbUrl: findMediaUrl(gif.files, false),
      fullUrl: findMediaUrl(gif.files, true) || findMediaUrl(gif.files, false),
    }))
    .filter((g: GifResult) => g.thumbUrl && g.fullUrl);
}
...
export default function GifPicker({
  ...
  async function fetchGifs(term: string) {
    ...
    try {
      const results =
        PROVIDER === "klipy"
          ? await fetchFromKlipy(term, controller.signal)
          : PROVIDER === "tenor"
          ...
          >
            ...
            <div className="px-5 pt-4">
              ...
              <p className="mt-2 font-mono text-[9px] uppercase tracking-widest text-muted">
                powered by {PROVIDER === "klipy" ? "Klipy" : PROVIDER === "tenor" ? "Tenor" : "GIPHY"}
              </p>
            </div>
