...
/**
 * Confirmed against a real Klipy response: each item has a `file` object
 * (singular — not `files`) with size buckets `xs`/`sm`/`md`/`hd`, each of
 * which has format keys `gif`/`webp`/`jpg`/`mp4`/`webm` → { url, width,
 * height, size }. `sm.gif` is a good thumbnail size; `md.gif` (falling
 * back to `hd.gif`) is a good full-size attach.
 */
async function fetchFromKlipy(term: string, signal: AbortSignal): Promise<GifResult[]> {
  ...
  return items
    .map((gif: any) => ({
      id: String(gif.id ?? gif.slug ?? Math.random()),
      title: gif.title || "gif",
      thumbUrl: gif.file?.sm?.gif?.url || gif.file?.xs?.gif?.url,
      fullUrl: gif.file?.md?.gif?.url || gif.file?.hd?.gif?.url || gif.file?.sm?.gif?.url,
    }))
