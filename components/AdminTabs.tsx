...
export default function AdminTabs({
  ...
  pollPanel: React.ReactNode;
  musicPanel: React.ReactNode;
  statusPanel: React.ReactNode;
}) {
  const [tab, setTab] = useState<"moderate" | "filter" | "announce" | "poll" | "music" | "status">("moderate");
    ...
    <div>
      <div className="mb-8 inline-flex flex-wrap gap-1 rounded-full border border-white/15 bg-night/60 p-1">
        ...
        <button
          onClick={() => setTab("poll")}
          className={`rounded-full px-5 py-2 text-sm font-bold transition ${
            tab === "poll" ? "bg-lime text-night" : "text-muted hover:text-paper"
          }`}
        >
          Poll
        </button>
      ...
      <div hidden={tab !== "poll"}>{pollPanel}</div>
