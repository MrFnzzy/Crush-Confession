...
import { REACTION_TYPES, REACTION_EMOJI, type ReactionType, type ReactionCounts, totalReactions } from "@/lib/reactions";

type Reply = { id: number; message: string; senderNickname: string | null; gifUrl?: string | null; isAdminReply?: boolean; createdAt: string };
type Confession = {
  id: number;
...
const REACTIONS_KEY = "unspoken_reactions";
...
function getMyReactions(): Record<string, ReactionType> { try { const raw = window.localStorage.getItem(REACTIONS_KEY); const parsed = raw ? JSON.parse(raw) : {}; return parsed && typeof parsed === "object" ? parsed : {}; } catch { return {}; } }
function saveMyReaction(id: number, type: ReactionType | null) {
  try {
    const current = getMyReactions();
    if (type === null) delete current[String(id)];
    else current[String(id)] = type;
    window.localStorage.setItem(REACTIONS_KEY, JSON.stringify(current));
  } catch {}
}
...
function ReactionPicker({
  ...
  function handleMainClick() {
    if (myReaction) {
      // Tapping the main button while already reacted removes the reaction,
      // matching a second tap on Facebook's Like button.
      onPick(myReaction);
    } else {
      setOpen((value) => !value);
    }
  ...
  return (
    <div ref={wrapRef} className="relative">
      <AnimatePresence>
        {open && (
          <motion.div
            ...
            className="absolute bottom-full left-0 z-10 mb-2 flex gap-1 rounded-full border border-night/10 bg-paper p-1.5 shadow-lg"
          >
            {REACTION_TYPES.map((type) => (
              <button
                ...
                title={type}
                className="grid h-9 w-9 place-items-center rounded-full text-xl transition hover:scale-125 active:scale-95"
              >
                {REACTION_EMOJI[type]}
              </button>
            ))}
      ...
      <button
        ...
        className={`rounded-full px-3 py-2 text-sm font-bold transition active:scale-95 ${myReaction ? "bg-coral text-paper" : "bg-night/5 hover:bg-coral hover:text-paper"}`}
      >
        {myReaction ? REACTION_EMOJI[myReaction] : "🤍"} {total > 0 ? total : ""} react
      </button>
    </div>
  );
}
...
export default function ConfessionCard({ confession, isAdmin = false, index = 0, displayNumber }: { confession: Confession; isAdmin?: boolean; index?: number; displayNumber?: number }) {
  ...
  const [hasRelated, setHasRelated] = useState<ReactionType | null>(null);
  const [reactionCounts, setReactionCounts] = useState<ReactionCounts>({
    like: confession.reactionLike ?? 0,
    love: confession.reactionLove ?? 0,
    haha: confession.reactionHaha ?? 0,
    wow: confession.reactionWow ?? 0,
    sad: confession.reactionSad ?? 0,
    angry: confession.reactionAngry ?? 0,
    crying: confession.reactionCrying ?? 0,
  });
  const [revealedName, setRevealedName] = useState<string | null>(null);
  const articleRef = useRef<HTMLElement | null>(null);

  useEffect(() => { setHasRelated(getMyReactions()[String(confession.id)] ?? null); }, [confession.id]);
  ...
  async function handleReaction(type: ReactionType) {
    const previous = hasRelated;
    const next: ReactionType | null = previous === type ? null : type;

    // Optimistic update: move the tally locally before the request resolves.
    ...
    setHasRelated(next);
    saveMyReaction(confession.id, next);

    try {
      const res = await fetch(`/api/confessions/${confession.id}/react`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ type: next, previousType: previous }),
      });
      if (!res.ok) throw new Error();
    } catch {
      // Roll back to the pre-request state.
      setReactionCounts((counts) => {
        const updated = { ...counts };
        if (next) updated[next] = Math.max(0, updated[next] - 1);
        if (previous) updated[previous] = updated[previous] + 1;
        return updated;
      });
      setHasRelated(previous);
      saveMyReaction(confession.id, previous);
  ...
  return (
    <motion.article ref={articleRef} initial={{ opacity: 0, y: 16 }} animate={{ opacity: deleting ? .4 : 1, y: 0, scale: deleting ? .98 : 1 }} transition={{ duration: .35, delay: Math.min(index, 8) * .04 }} className="relative overflow-hidden rounded-2xl border border-night/10 bg-paper p-5 text-night shadow-[8px_8px_0_#ff4d6d] sm:p-7">
      ...
      )}</div>{isAdmin && <button onClick={() => deleteReply(reply.id)} disabled={busy} className="shrink-0 font-mono text-[10px] uppercase text-coral hover:underline">remove</button>}</div><span className="font-mono text-[10px] uppercase tracking-wide text-night/40">— {reply.senderNickname || "anonymous"} · {timeAgo(reply.createdAt)} {reply.isAdminReply && <span className="ml-1 rounded-full bg-night px-2 py-0.5 font-mono text-[9px] font-bold uppercase tracking-widest text-lime">✔ admin</span>}</span></li>)}</ul>}

      <div className="mt-7 flex flex-wrap items-center gap-2 border-t border-night/10 pt-4"><ReactionPicker myReaction={hasRelated} counts={reactionCounts} onPick={handleReaction} /><button onClick={() => setShowReplyForm((value) => !value)} className="rounded-full px-3 py-2 text-sm font-bold text-coral transition hover:bg-coral/10">{showReplyForm ? "cancel" : "reply →"}</button><NotifyBell confessionId={confession.id} />{isAdmin && <button onClick={deleteConfession} disabled={deleting} className="rounded-full px-3 py-2 text-sm text-night/45 hover:bg-night/5">{deleting ? "deleting..." : "delete"}</button>}</div>
