...
export default function ConfessionCard({
  ...
  index?: number;
  /**
   * The number shown on the card's badge (e.g. #003). Deliberately
   * separate from confession.id: ids have gaps once older confessions
   * are deleted, so numbering by id would jump around (#001, #005,
   * #006...). This is instead each confession's rank by creation order
   * among what's currently on the wall, so the numbers always run
   * cleanly from 001 with no gaps. Falls back to the raw id if the
    ...
    >
      <div className="flex items-start justify-between gap-3">
        <span className="inline-block rounded-full bg-ink px-3 py-1 font-mono text-xs text-paper">
          #{String(displayNumber ?? confession.id).padStart(3, "0")}
        </span>
