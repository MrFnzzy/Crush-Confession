...
import { sendReplyPush } from "@/lib/push";
...
export async function POST(
  ...
  const isAdminReply = isAdminFromCookieValue(req.cookies.get(ADMIN_COOKIE_NAME)?.value);
  ...
  try {
    ...
    const reply = await prisma.reply.create({
      data: { message, senderNickname, gifUrl, confessionId, isAdminReply },
    });

    try {
      await sendReplyPush(confessionId, message || (gifUrl ? "sent a gif" : ""));
    } catch (err) {
      // A push failure should never fail the reply itself.
      console.error("Failed to send reply push:", err);
    }
