...
export default async function AdminPage() {
  ...
  const [{ confessions, failed }, visitCount, customBannedWords] = await Promise.all([
    getConfessions(),
    getVisitCount(),
  ...
  const moderatePanel = (
    <>
      ...
      <section aria-label="Dashboard metrics" className="mb-10 grid gap-4 sm:grid-cols-3">
  ...
  return (
    <>
      ...
      <main className="site-grid min-h-screen px-4 py-10 sm:px-8 sm:py-16"><div className="mx-auto max-w-7xl">
        <AdminTabs moderatePanel={moderatePanel} filterPanel={filterPanel} />
      </div></main>
    </>
  );
}
