...
export default function ConfessPage() {
  ...
  async function handleSubmit(event: FormEvent) {
    ...
    setSubmitting(true);
    try {
      const res = await fetch("/api/confessions", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ crushName: crushName.trim(), message: message.trim(), senderNickname: senderNickname.trim() || undefined }) });
      if (!res.ok) { const data = await res.json().catch(() => ({})); throw new Error(data.error || "Something went wrong. Try again."); }
        ...
        <div className="mx-auto max-w-7xl">
          ...
          <div className="mt-10 grid gap-12 lg:grid-cols-[.9fr_1.1fr] lg:items-start">
            ...
            <motion.div initial={{ opacity: 0, y: 18 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: .08 }} className="grid gap-6 md:grid-cols-[1.05fr_.75fr]">
              ...
              <div className="hidden md:block"><p className="mb-3 font-mono text-[10px] uppercase tracking-widest text-muted">live preview / unfiltered</p><div className="relative rounded-2xl border border-white/10 bg-violet p-6 text-paper shadow-[8px_8px_0_#d9ff54]"><span className="font-mono text-[10px] uppercase tracking-widest text-paper/60">signal #— / just now</span><p className="mt-14 font-mono text-[10px] uppercase tracking-widest text-lime">to: {crushName.trim() || "someone"}</p><p className="mt-4 min-h-[8rem] whitespace-pre-wrap break-words font-display text-3xl leading-[.95]">{message.trim() || "Your words will show up here."}</p><p className="mt-10 font-mono text-[10px] uppercase tracking-widest text-paper/60">— {senderNickname.trim() || "anonymous"}</p></div></div>
            </motion.div>
          </div>
        </div>
      </main>
