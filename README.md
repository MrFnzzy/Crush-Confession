...
Also includes:
- **Gif picker** — attach a gif from a searchable library when composing a confession.
...
- `/wall` — every confession, numbered in the order they were posted, each
  with a ♡ "relate" button (one tap per browser, tracked in localStorage) and
  a reply box anyone can use, also with an optional nickname. A toggle at the
  top switches between "Newest" and "Most related".
